...
- **[DEBT] register stdout leaks macOS-only "launchd job" string trên Linux** (Phase 3.5+ Tầng 2) — from dogfood probe 2026-05-28. `advisory-cron register` trên Linux prints `"registered launchd job: com.advisorycron.<label>"` — sai vì launchd không tồn tại trên Linux. Hardcoded message ở `core::register::run` (or `cli::register` render layer). Fix: OS-conditional render — Linux say `"registered cron entry: # advisory-cron: <label>"` (mirrors INV-22 tag). ~5 LOC. P013 EXECUTE missed cosmetic surface.

- **[DEBT] status render Linux vẫn macOS-flavored** (Phase 3.5+ Tầng 2) — from dogfood probe 2026-05-28. `advisory-cron status` trên Linux prints `"Plist: loaded"` + `"Next fire: unknown (launchctl format not recognized)"`. P013 DP5 chỉ closed REGISTER render gate (`src/cli/register.rs::plist_path` → "(cron-tab managed)"). STATUS CLI render (`src/cli/status.rs` or `src/core/status.rs`) vẫn dùng macOS labels + tries `parse_next_fire` macOS launchctl descriptor parser on Linux output. Fix: (a) rename "Plist" → "Schedule entry" (OS-agnostic), (b) Linux-specific `next_fire` parser from cron expression (Linux M H * * * already daily-form per INV-23). ~15-20 LOC. Tied to Discovery 1 above — likely same hotfix phiếu.
